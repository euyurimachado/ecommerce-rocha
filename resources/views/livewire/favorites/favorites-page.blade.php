<div>
    @if ($products->isEmpty())
        <div class="rounded-lg border border-slate-200 bg-white p-6 text-center shadow-sm">
            ...
            <h1 class="mt-4 text-xl font-bold text-slate-950 md:text-2xl">Sua lista está vazia</h1>
            <p class="mt-2 text-sm text-slate-600">Favorite produtos para montar sua próxima compra com mais agilidade.</p>
            <a href="{{ route('home') }}" class="mt-5 inline-flex rounded-lg bg-rocha-blue px-5 py-3 font-bold text-white transition hover:bg-rocha-blue-dark">Explorar produtos</a>
        </div>
    @else
        <div class="flex items-end justify-between gap-4">
            <div>
                <p class="text-sm font-bold text-rocha-blue">Favoritos</p>
                <h1 class="mt-1 text-2xl font-bold text-slate-950 md:text-3xl">Produtos salvos</h1>
