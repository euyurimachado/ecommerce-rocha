<article class="group flex h-full flex-col overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:border-sky-200 hover:shadow-md">
    <a href="{{ route('products.show', $product) }}" class="block">
        <div class="flex aspect-square items-center justify-center bg-slate-100 p-5">
            <div class="grid size-28 place-items-center rounded-lg border border-slate-200 bg-white text-center shadow-inner transition group-hover:border-sky-200">
    ...
    <div class="flex flex-1 flex-col p-4">
        <div class="flex items-center justify-between gap-2 text-xs">
            ...
            <span class="inline-flex items-center gap-1 text-slate-500"><x-rocha-icon name="star" class="size-3.5 text-sky-500" />{{ $product->rating }}</span>
        </div>
