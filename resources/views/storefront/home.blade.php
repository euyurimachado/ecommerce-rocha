...
@section('content')
        ...
        <div class="mx-auto max-w-7xl px-4 py-4 lg:px-6">
            <div class="flex gap-3 overflow-x-auto pb-1">
                @foreach ($categories as $category)
                    <a href="{{ route('categories.show', $category) }}" class="flex min-w-fit items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-800 transition hover:border-sky-200 hover:bg-sky-50 hover:text-sky-800">
                        <span class="grid size-8 place-items-center rounded-md bg-sky-600 text-xs text-white shadow-sm">{{ $category->icon }}</span>
                        {{ $category->name }}
                    </a>
                @endforeach
            </div>
    ...
    <section class="bg-slate-950 text-white">
        <div class="mx-auto grid max-w-7xl gap-6 px-4 py-8 md:grid-cols-[1.5fr_0.8fr] lg:px-6 lg:py-10">
            <div class="relative overflow-hidden rounded-lg border border-white/10 bg-slate-900 p-6 shadow-xl md:p-8">
                <div class="absolute right-0 top-0 h-full w-1/2 bg-[radial-gradient(circle_at_top_right,rgba(14,165,233,0.28),transparent_58%)]"></div>
                <div class="relative">
                <p class="inline-flex items-center gap-2 text-sm font-bold text-sky-300"><x-rocha-icon name="sparkles" class="size-4" /> Rocha Sports em Campos dos Goytacazes</p>
                ...
                <div class="mt-6 flex flex-wrap gap-3">
                    <a href="#ofertas" class="inline-flex items-center gap-2 rounded-lg bg-sky-500 px-5 py-3 text-sm font-black text-white transition hover:bg-sky-400"><x-rocha-icon name="tag" class="size-4" />Ver ofertas</a>
            ...
            <div class="grid gap-3 sm:grid-cols-3 md:grid-cols-1">
                <div class="rounded-lg border border-white/10 bg-white/5 p-4">
                    <p class="flex items-center gap-2 font-black"><x-rocha-icon name="shield-check" class="size-5 text-sky-300" />Pagamento seguro</p>
                    <p class="mt-1 text-sm text-slate-300">Pix, cartão e checkout protegido.</p>
                </div>
                <div class="rounded-lg border border-white/10 bg-white/5 p-4">
                    <p class="flex items-center gap-2 font-black"><x-rocha-icon name="truck" class="size-5 text-sky-300" />Entrega rápida</p>
                    <p class="mt-1 text-sm text-slate-300">Operação local em Campos.</p>
                </div>
                <div class="rounded-lg border border-white/10 bg-white/5 p-4">
                    <p class="flex items-center gap-2 font-black"><x-rocha-icon name="badge-check" class="size-5 text-sky-300" />Produtos originais</p>
    ...
    <section id="mais-pedidos" class="mx-auto max-w-7xl px-4 py-8 lg:px-6">
        <div class="flex items-end justify-between gap-4">
            ...
            <a href="{{ route('search', ['ordenar' => 'mais-vendidos']) }}" class="inline-flex items-center gap-1 text-sm font-bold text-sky-700">Ver todos <x-rocha-icon name="chevron-right" class="size-4" /></a>
        </div>
    ...
    <section id="ofertas" class="bg-white">
        <div class="mx-auto max-w-7xl px-4 py-8 lg:px-6">
            <div class="flex items-end justify-between gap-4">
                ...
                <a href="{{ route('search', ['ordenar' => 'ofertas']) }}" class="inline-flex items-center gap-1 text-sm font-bold text-sky-700">Ver ofertas <x-rocha-icon name="chevron-right" class="size-4" /></a>
            </div>
    ...
    <section class="mx-auto max-w-7xl px-4 py-8 lg:px-6">
        ...
        <div class="mt-5 grid grid-cols-2 gap-4 md:grid-cols-5">
            @foreach ($brands as $brand)
                <div class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-sky-200 hover:shadow-md">
