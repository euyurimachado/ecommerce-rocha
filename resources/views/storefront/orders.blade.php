    ...
    <section class="mx-auto max-w-5xl px-4 py-8 lg:px-6">
        <div class="grid gap-6 lg:grid-cols-[0.9fr_1.4fr]">
            <aside class="h-fit rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                <p class="text-sm font-bold text-rocha-blue">Histórico de pedidos</p>
                <h1 class="mt-2 text-3xl font-black text-slate-950">Acompanhe suas compras</h1>
                ...
                <form action="{{ route('orders.index') }}" method="GET" class="mt-5 space-y-3">
                    ...
                    <button class="flex w-full items-center justify-center gap-2 rounded-lg bg-rocha-blue px-5 py-3 font-black text-white transition hover:bg-rocha-blue-dark" type="submit">
            ...
            <div>
                @if ($contact === '')
                    <div class="rounded-lg border border-slate-200 bg-white p-6 text-center shadow-sm">
                        ...
                        <h2 class="mt-4 text-xl font-black text-slate-950">Consulte seu histórico</h2>
                ...
                @elseif ($orders->isEmpty())
                    <div class="rounded-lg border border-slate-200 bg-white p-6 text-center shadow-sm">
                        ...
                        <h2 class="mt-4 text-xl font-black text-slate-950">Nenhum pedido encontrado</h2>
                        <p class="mt-2 text-sm text-slate-600">Confira o contato informado ou faça uma nova compra na loja.</p>
                        <a href="{{ route('home') }}" class="mt-5 inline-flex rounded-lg bg-rocha-blue px-5 py-3 font-black text-white transition hover:bg-rocha-blue-dark">Voltar para a loja</a>
                    </div>
                @else
                    <div class="space-y-4">
                            ...
                            <article class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                                <div class="flex flex-wrap items-start justify-between gap-4">
                                    <div>
                                        ...
                                        <h2 class="mt-1 text-xl font-black text-slate-950">{{ $order->status_label }}</h2>
                                        <p class="mt-1 text-sm text-slate-500">{{ $order->created_at->format('d/m/Y H:i') }} - {{ $order->fulfillment_method_label }}</p>
                                    </div>
                                    <p class="rounded-lg bg-slate-50 px-4 py-2 text-sm font-black text-slate-950">{{ $order->formatted_total }}</p>
                                </div>
                                ...
                                <a href="{{ route('orders.status', ['order' => $order->code]) }}" class="mt-5 inline-flex items-center gap-2 rounded-lg border border-slate-200 px-4 py-2 text-sm font-black text-slate-700 transition hover:border-rocha-blue/30 hover:bg-rocha-blue/5 hover:text-rocha-blue">
                                    Ver detalhes
